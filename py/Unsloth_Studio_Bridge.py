...
# the companion thinking display node can render them in real time.
...
def _request_json(url, body=None, timeout=HTTP_TIMEOUT, retries=1, rediscover_url=None, headers=None):
    data = None
    method = "GET"
...
def _strip_studio_api_path(base_url):
    """Studio's UI and its /api share a port; the llama-compatible endpoints
    (/props, /v1/chat/completions) live on the root, so drop a trailing /api."""
...
def _probe_studio(base_url, timeout=STUDIO_PROBE_TIMEOUT):
    """Root URL when this base answers as an Unsloth Studio server, else None.
    Studio serves /api/liveness and /api/health unauthenticated; a bare
    llama-server does not have either."""
    root = _strip_studio_api_path(base_url)
...
def _studio_credential(base_url, credential):
    """Bearer for Studio's llama-compatible endpoints: the node's credential,
    or a token minted from this machine's Studio install when it is local.
    A remote Studio has no local auth files, so it needs the node's key."""
    if str(credential or "").strip():
        return str(credential).strip()
    if _is_loopback_url(base_url):
        token = _get_studio_token(
            f"{_strip_studio_api_path(base_url)}/api", STUDIO_ADMIN_USERNAME, ""
        )
        return token or ""
    return ""


def _server_ready(base_url, credential=""):
    """True when the target can answer a chat: llama-server serves /health, a
    Studio base serves /props with a model resident."""
    ...
    try:
        ...
        return False
    return bool(props.get("default_generation_settings") or props.get("model_path"))
...
def _studio_target(studio_root, credential):
    """Build the target for an Unsloth Studio server, rejecting it when no
    model is resident so the caller's recovery path can reload one."""
    studio_api_url = f"{studio_root}/api"
    effective = _studio_credential(studio_root, credential)
    try:
        props = _request_json(
            f"{studio_root}/props",
            timeout=HTTP_TIMEOUT,
            retries=0,
            headers=_auth_headers(effective),
        )
    except Exception as exc:
        raise RuntimeError(
            f"Unsloth Studio at {studio_root} did not accept the request ({exc}). "
            "Set studio_api_key, or enable keyless API access in Studio's settings."
        ) from exc
    if not (props.get("default_generation_settings") or props.get("model_path")):
        raise RuntimeError(
            f"Unsloth Studio at {studio_root} is running but has no model loaded. "
            "Load a model in Studio and retry."
        )
    return studio_root, "configured Unsloth Studio URL", studio_api_url, effective


def _resolve_llama_server(configured_url, credential=""):
    """Resolve the llama-compatible base URL for this run.

    Returns (base, source, studio_api_url, credential). studio_api_url is None
    for a raw llama-server; for a Studio base it is Studio's own API URL and
    credential is the bearer that works against the llama-compatible endpoints."""
    ...
    if configured_base != default_base:
        studio_root = _probe_studio(configured_base)
        if studio_root is not None:
            return _studio_target(studio_root, credential)
        if _is_server_alive(configured_base):
            return configured_base, "configured llama-server URL", None, credential
        if not _is_loopback_url(configured_base):
            raise RuntimeError(
                f"Could not reach Unsloth Studio or llama-server at {configured_base}. "
                "Check the address, and that Studio is running with a model loaded."
            )
    ...
    return detected_base, f"Studio log discovery (port {llama_port})", None, credential
...
def _token_expired(token):
    """True for a JWT past its exp. Studio sessions last an hour, so a cached
    one outlives a long ComfyUI session and must be re-obtained, not reused."""
    parts = token.split(".")
    if len(parts) != 3:
        return False
    try:
        ...
        return False
    exp = claims.get("exp")
    return isinstance(exp, (int, float)) and exp <= time.time() + 5
...
def _get_studio_token(studio_api_url, username, credential):
    ...
    if _STUDIO_TOKEN:
        if not _token_expired(_STUDIO_TOKEN):
...
def _unload_model_via_studio(studio_api_url, username, credential, model_path, base_url=None):
    global _STUDIO_TOKEN
    if not _is_loopback_url(studio_api_url) and not str(credential or "").strip():
        print(
            "[Unsloth Studio Bridge] WARNING: remote Studio unload requires "
            "studio_api_key; skipping.",
            file=sys.stderr,
        )
        return False
    token = _get_studio_token(studio_api_url, username, credential)
...
def _recover_llama_server(configured_url, studio_api_url, username, credential):
    """Reload the last loaded model via Studio when the target is down
    ...
    attempt; returns a resolved target tuple or None when exhausted. Fails
    within seconds when Unsloth Studio itself is not running."""
    global _LAST_MODEL_PATH
    if not _is_loopback_url(studio_api_url) and not str(credential or "").strip():
        raise RuntimeError(
            f"Unsloth Studio at {studio_api_url} needs studio_api_key to reload "
            "or unload the model."
        )
    ...
    for cycle in range(1, RECOVERY_MAX_CYCLES + 1):
        try:
            target = _resolve_llama_server(configured_url, credential)
            if _server_ready(target[0], target[3]):
                print(
                    f"[Unsloth Studio Bridge] Server recovered: {target[0]} ({target[1]})",
                    file=sys.stderr,
                )
                _remember_launch(target[0])
                return target
        except Exception:
            pass
        if not _studio_reachable(studio_api_url):
        ...
        if not _reload_model_via_studio(studio_api_url, username, credential, model_path):
            raise RuntimeError(
                f"Could not reload the model through Unsloth Studio at "
                f"{studio_api_url}. Check studio_api_key and that a model is loaded."
            )
...
def _get_server_state(base_url, credential=""):
    props = _request_json(f"{base_url}/props", headers=_auth_headers(credential))
...
def _count_chat_tokens(base_url, messages, disable_thinking, studio_api_url=None, credential=""):
    if studio_api_url:
        # Studio has no /apply-template or /tokenize; its own counter renders
        # the same chat template and reports the same prompt-token count.
        try:
            result = _request_json(
                f"{studio_api_url.rstrip('/')}/inference/chat/count_tokens",
                {
                    "model": "default",
                    "messages": messages,
            ...
            print(
                f"[Unsloth Studio Bridge] WARNING: Studio token count unavailable "
                f"({exc}); skipping the context budget check.",
                file=sys.stderr,
            )
            return None
...
def _chat_completion(base_url, messages, seed, params, include_reasoning, disable_thinking, cache_skills=True, configured_url=None, studio_credential="", studio_api_url=None):
    body = {
        "model": "default",
        "messages": messages,
        "seed": seed,
    ...
    def _recover_url():
        target = _recover_llama_server(
            configured_url,
            studio_api_url or _discover_studio_api_url(),
    ...
    result = _request_json(
        ...
        headers=_auth_headers(studio_credential),
    )
...
def _chat_completion_stream(base_url, messages, seed, params, disable_thinking,
    ...
    request_headers.update(_auth_headers(studio_credential))
    req = urllib.request.Request(
        f"{base_url}/v1/chat/completions",
        data=json.dumps(body).encode("utf-8"),
        headers=request_headers,
...
class UnslothLLM:
                ...
                "unsloth_server_url": (
                    "STRING",
                    {
                        ...
                        "tooltip": (
                            ...
                            "port from its logs. Or enter an Unsloth Studio base URL "
                            "(http://host:port or http://host:port/api) to use Studio's "
                            "llama-compatible endpoints directly; a remote Studio needs "
                            "studio_api_key, or keyless API access enabled in Studio. A raw "
                            "llama-server base URL also works. Studio must have a model loaded; "
                ...
                "studio_api_key": (
                    "STRING",
                    {
                        ...
                        "tooltip": (
                            ...
                            "automatically on this machine; a remote Studio always needs "
                            "a key or password here. Used for chat when Studio requires "
                            "auth, for unload after run, and for auto-reload of the last model."
                        ),
                    },
                ),
    ...
    def generate(self, prompt, seed, unsloth_server_url=DEFAULT_UNSLOTH_STUDIO_URL, skills_path="", disable_thinking=True, include_reasoning=False, cache_skills=True, retain_last_response=0, image=None, temperature=-1.0, top_p=-1.0, top_k=-1, disable_web_search=False, url_web_search="", studio_api_key="", unload_model_after_run=False, live_display=True, unique_id=None):
        ...
        configured_base = _normalize_server_url(unsloth_server_url)
        if configured_base != _normalize_server_url(DEFAULT_UNSLOTH_STUDIO_URL):
            studio_root = _probe_studio(configured_base)
            if studio_root is not None:
                studio_api_url = f"{studio_root}/api"
        try:
            llama_base, connection_source, studio_target_url, studio_credential = _resolve_llama_server(
                unsloth_server_url, studio_api_key
            )
        except Exception:
            recovered = _recover_llama_server(
                unsloth_server_url,
                studio_api_url,
                STUDIO_ADMIN_USERNAME,
                studio_api_key,
            ...
            if recovered is None:
                raise
            llama_base, connection_source, studio_target_url, studio_credential = recovered
            connection_source = "recovered after crash"
        if studio_target_url is not None:
            studio_api_url = studio_target_url
        ...
        server_state = _get_server_state(llama_base, studio_credential)
        ...
        if live_display:
            ...
            try:
                stream_content, stream_reasoning, stream_stats = _chat_completion_stream(
                    ...
                    studio_credential=studio_credential,
        ...
        if unload_model_after_run:
            _unload_model_via_studio(
                ...
                base_url=None if studio_target_url is not None else llama_base,
            )
        return (response,)
